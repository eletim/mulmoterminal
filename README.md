...
- **Scroll speed** — one wheel notch or trackpad swipe moves the terminal the same distance
  ...
  Persistent terminals keep a small per-viewer history cache. After Core resolves foreground-app
  ownership, ordinary small scrolls reuse xterm's already-parsed local window without rewriting
  the screen, and only fetch from the session again near a cached edge.
