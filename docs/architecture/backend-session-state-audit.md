...
| `hookedSessions` | Whether this exact launch carries Claude hooks | Process-local; never pruned | Claude spawner, MCP history gate | Core capability metadata | Move to Core | Agent identity alone is insufficient because a launcher may run Claude without hook settings. The explicit Core fact survives restart and expires on Delete. |
...
| Core adapter `inputTails`, `exitWatchers`, `capabilityTails` | Serialized input/metadata and native-exit observers | Process-local | Core adapter | Keep; tails self-remove, watchers dispose or are removed on exit/Delete. |
| Core resume-source retry (`core-resume-source.ts`) | Bounded retry of one durable Core metadata write | Process-local Promise only | Agent resume discovery | Keep; no cache, map, or fallback source is retained after completion. |
