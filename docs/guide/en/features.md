...
| GUI panel | Renders diagrams, forms, images, documents, HTML, and **video/slides (MulmoCast)** from the agent's tool calls (Claude / Codex both supported) |
| Clickable file paths | A **file path an agent prints** in the terminal becomes a link, and **what it opens is chosen by extension**: `.md` renders, `.json` is indented, `.csv` / `.tsv` become a table (each in a new tab), source and `.txt` open in the app's own read-only **Files** view, and images / PDF / video open as-is. **While a grid cell is enlarged the file pane beside it takes the click instead**, so the file opens next to the terminal that printed it — everything but the images / PDF / video row, and only for files under that cell's own directory. |
| Files | A read-only directory and file viewer, opened from the toolbar or a terminal/session cwd |
...
### Viewing files beside a terminal

Enlarging a grid cell (**Expand**) puts a **folder** toggle in its header. It splits the enlarged
area in two: terminal on the left, the file viewer on the right, rooted at that
...
The same viewer still opens full-screen from a **Files** header button or by clicking a file
...
Files is read-only. It does not save, rename, delete, or behave as an IDE editor; it is for
checking files that belong to the terminal session's working directory.
...
| Launchers / cwd presets | Extend launch commands and working-directory suggestions in settings |
...
| Persistent history scrolling | Wheel through long Shell history without entering tmux copy-mode. Each browser tab keeps an independent opaque viewport cursor; reconnect starts live, and full-screen applications receive scroll through the Core session API |
