...
拡大していないグリッドには「今のターミナル」が存在しないので、推測せず何もしません。拡大の移動は
...
| 組み合わせ | 理由 |
|---|---|
| `Cmd`/`Ctrl`+`W`・`+T`・`+N`・`Cmd`/`Ctrl`+`Shift`+`T` | **ブラウザの予約キー**（タブを閉じる／新規タブ／新規ウィンドウ）。ページ側から横取りできず、割り当てても**何も起きません** |
...
**既定では `F1`〜`F12` はブラウザに届きません。** macOS は最上段を*メディアキー*（明るさ・音量など）
として扱うため、`F2` に割り当てても**完全に無反応**に見えます。keydown 自体が配送されないので、
MulmoTerminal 側からは検知すらできません。対処は2つ。

- **`Fn`+`F2`** を押す。本来のファンクションキーが送られ、`"F2"` の割り当てにマッチします。`Fn` は
  ブラウザが報告する修飾キーではないので、割り当て文字列に書く必要はありません。
- または **システム設定 → キーボード → 「F1、F2 などのキーを標準のファンクションキーとして使用」** を ON。
  素のキーで効くようになり、`Fn` 併用が逆にメディア操作になります。

**`F4` は避けたほうが無難**です。macOS が Spotlight / Launchpad に割り当てているため、上記の設定を
変えても飲み込まれることがあります。

**`Option`+英字は macOS では当てになりません。** 割り当てはブラウザの `KeyboardEvent.key` と照合されますが、
macOS は Option 併用時に英字ではなく**合成後の文字**を報告します（`Option`+`n` は通常デッドキーの `˜` として
届き、`n` にはなりません）。そのため `"Alt+n"` は一致しません。Option を使うなら
**印字されないキー**（`Alt+ArrowDown`・`Alt+PageUp` など）と組み合わせてください。こちらは影響を受けません。

{: .note }
> そのキーが実際に何を送っているか分からないときは、ブラウザの devtools コンソールに次を貼って押してみて
> ください。**何も出力されなければ、ページに届く前に OS かハードウェアが奪っています。**
>
> ```js
