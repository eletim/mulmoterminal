...
**ディレクトリごと**の指定（[後述](#per-dir)）はこれとは違い、ファイルが監視されているので保存した
瞬間に反映されます。
...
- **どのフォントを指定しても何も変わらない。** サーバを再起動していない可能性が高いです。グローバル
  設定は起動時にしか読まれません（上記参照）。ディレクトリごとの `fontFamily` なら再起動は不要です。
...
グローバル側と違い、こちらは**再起動不要**です。`.mulmoterminal.json` は監視されているので、保存した
瞬間にそのディレクトリのターミナルが新しいフォントで再フィットします。
