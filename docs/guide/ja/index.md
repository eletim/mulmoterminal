...
IDE や分割ターミナルから移った方々が、**何から逃げてきて、実際に何が変わったか**。

### 「メモリを食わなくなった」

エージェントを分けるために IDE のウィンドウを分けると、**1体につきエディタ・言語サーバー・拡張機能・
ファイルウォッチャーが一式**増えます。**64GB のマシンでも足りなくなる**のはそのためです。

こちらはエージェントがサーバー側の PTY で、UI はブラウザのタブなので、カクついていたマシンが
サクサク動くようになった、という報告が複数あります。

### 「別のエージェントに返信しなくなった」

**6枚のペインは全部同じに見えます。** 「別のエージェントのターミナルに返事を打ち込んでいた」
「そもそも何を頼んだんだっけ、を思い出せない」——注意力の問題ではなく、
**同じ見た目のペインがN枚あると、N個の文脈を頭の中に持ち続けなければならない**という問題です。

**状態の色・名前バッジ・ディレクトリごとの色**は、それを頭から画面に移すためのものです。

### 「たくさん見ることと、1つをちゃんと読むことが、両立するようになった」

ターミナルを6分割すると、1つ1つが小さすぎて **4000字の返答をスクロールとリサイズなしには読めません**。
だから人は、エージェントを1体増やすたびに「読みにくさ」を黙って受け入れています。
