...
`npx mulmoterminal` を実行したディレクトリがそれで（`--cwd` で上書き）、
起動時に `Workspace: …` と表示されます。
...
**3.x の単一ビューでしていたことを続けるなら、Claude をワークスペースで起動します。**
...
> そこを既定にするには、そのディレクトリで `npx mulmoterminal` を実行するか、
> `--cwd ~/mulmoclaude` を渡します。
...
ワークスペースで起動しても、GUI ツールはそのディレクトリに登録したぶんだけです。
Canvas に描かせたい・コレクションを触らせたいときは、
ランチャの MCP トグルを必要な分だけ入れてください。
トグルは**セッションではなくディレクトリ**への登録なので、次にそこで起動するセッションにも効きます。
