...
## レビューで直したところ: 設定一覧の行キー (#1023, Codex)

指摘は正しかった。設定一覧の `v-for` に**バインド文字列をそのままキーに使っていた**。

同じキーストロークを 2 つ書くのは**通る**（`validateKeymap` は警告するだけで、`sanitizeKeymap` も
両方残す）。すると Vue が同じキーを 2 度見て DOM ノードを再利用し、**片方が画面から消える** —
「何を設定したか」を見せるための唯一の画面で、設定した行が消えるのが一番まずい。

一意性を `SendRow.id`（`${index}:${key}`）として**純粋モジュール側に持たせた**。テンプレートの
式に埋めると単体テストで押さえられないため（CLAUDE.md の「純粋ロジックは mount しないで
テストする」に沿う形）。

対照実験で確認済み: `id` をキー文字列に戻すと 3 本が赤、戻すと緑。

なお**重複した行も両方表示している**。どちらが発火するかを報告するのは検証側の仕事で、
一覧は「設定ファイルに何が書いてあるか」を見せる場所という切り分け。発火しない方を
グレーアウトする案は考えたが、今回のスコープ外として PR に書いて判断を仰いだ。
