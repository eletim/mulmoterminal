...
function setup(terminalModes: readonly number[] = []) {
  ...
  // Kept on the runtime object even though it is deliberately absent from ConnectionDeps. This
  // is the old Core/send-keys pane-injection route; asserting it stays untouched prevents the
  // interactive transport from crossing that boundary again (#157).
...
describe("handleClientFrame", () => {
  ...
  it.each([
    ...
    ["raw control and UTF-8 input", "\x1b[200~λ\0\x1b[201~"],
