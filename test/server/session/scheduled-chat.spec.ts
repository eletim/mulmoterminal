...
async function fresh() {
  ...
  return { history, spawnScheduledWorker, runCompletionHook };
}
...
describe("a scheduled task's chat", () => {
  it("creates a background Core session, not a normal user terminal", async () => {
    ...
    expect(spawn).toHaveBeenCalledWith(ID, "background", "scheduled");
  });
  ...
  it("retains background history while Core origin carries the push exception", async () => {
    const { history, spawnScheduledWorker } = await fresh();
    spawnScheduledWorker(ID, noop);
    expect(history.isBackgroundHistory(ID)).toBe(true);
  });
  ...
  it("still says so when it fails", async () => {
    ...
    // finished turn.
    const { history, spawnScheduledWorker, runCompletionHook } = await fresh();
    spawnScheduledWorker(ID, noop);
