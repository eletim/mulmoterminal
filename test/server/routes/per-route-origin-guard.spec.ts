...
// The four readers that are NOT Express routes, and cannot use the helper:
...
//   terminal-control-server.ts
//                         socket.io's own handshake/CORS hooks for the control socket
const ORIGIN_READERS = new Set(["routes/same-origin-guard.ts", "routes/ws-routes.ts", "infra/pubsub.ts", "control/terminal-control-server.ts"]);
