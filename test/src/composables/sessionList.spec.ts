...
import { sessionAttention, sessionDot } from "../../../src/composables/sessionList";
...
describe("sessionDot", () => {
  it("gives blocked amber and done green, with a label for each", () => {
    expect(sessionDot("blocked")).toEqual({ cls: expect.stringContaining("bg-[#f59e0b]"), label: "Waiting for you" });
    expect(sessionDot("done")).toEqual({ cls: expect.stringContaining("bg-[#22c55e]"), label: "Finished — unread" });
  });

  // `working` owns the same slot with its spinner, and the two states cannot co-occur; `idle` has
  // nothing to report.
  it("has no dot for working or idle", () => {
    expect(sessionDot("working")).toBeNull();
    expect(sessionDot("idle")).toBeNull();
  });
  ...
  it("never returns a dot without an accessible label", () => {
    for (const status of ["blocked", "done", "working", "idle"] as const) {
      const dot = sessionDot(status);
      if (dot) expect(dot.label.length).toBeGreaterThan(0);
    }
  });
  ...
  it("carries no animation utility", () => {
    for (const status of ["blocked", "done"] as const) {
      expect(sessionDot(status)?.cls).not.toMatch(/animate-/);
    }
  });
});
