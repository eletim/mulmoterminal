...
//
// `waiting` is checked BEFORE `working` for Notification because it never clears `working`:
...
export function reapDecisionFor(activity: ReapActivity | undefined, graces: ReapGraces): ReapDecision {
  if (activity?.waiting && activity.event === "Notification") return { kind: "arm", delayMs: graces.waitingMs };
