...
import { decodeImageDataUrl, pasteImageFilename, pasteImageToken, prunePasteImages, PASTE_IMAGE_DIR } from "./paste-image-store.js";
...
export function mountPasteImageRoute(app: Express, { dir = PASTE_IMAGE_DIR, now = () => new Date(), token = pasteImageToken }: PasteImageOptions = {}): void {
  app.post("/api/paste-image", async (req, res) => {
    ...
    const filename = pasteImageFilename(now(), decoded.mime, token());
