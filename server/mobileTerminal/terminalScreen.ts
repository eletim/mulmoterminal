...
// The second bound, and the reason it exists: before the history the row count alone kept the
// screen small; it no longer does.
...
// Exported so ansiSegments.ts's styled window (#7) applies the SAME ceiling rather than growing
// unboundedly on a wide pane. A large local HTTP response is still a real cost on a phone's
// connection.
...
// each field it receives as its own labelled row, so an empty one would read as "this session has
// no branch" instead of "not known".
