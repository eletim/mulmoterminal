...
<template>
  <div class="fixed inset-0 z-[100] flex items-center justify-center bg-[rgba(0,0,0,0.55)]" @click.self="emit('close')">
    ...
    >
      ...
      <h3 class="mb-2 mt-3.5 text-[12px] font-semibold uppercase tracking-[0.04em] text-muted">Waiting rows</h3>
      <p class="mb-2 mt-1.5 text-[12px] text-dim">
        In the list beside an enlarged cell, a row whose agent is <strong>waiting on you</strong> — a permission prompt, a question — carries an amber edge and
        wash, and blinks. A row that has simply <strong>finished</strong> is green and holds still. Turning this off keeps both colours and stops the movement;
        rows never blink when your system asks for reduced motion.
      </p>
