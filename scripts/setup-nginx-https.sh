...
write_if_changed() {
  ...
  if [[ -f "$file" ]] && cmp -s "$tmp" "$file"; then
    echo "[mulmoterminal] unchanged ${file}"
    rm -f "$tmp"
    return 0
  fi
...
install_include_once() {
  ...
  if grep -qF "$marker_begin" "$server_conf"; then
    echo "[mulmoterminal] include already present in ${server_conf}"
    return 0
  fi

  local tmp backup
  tmp="$(mktemp)"
  if ! awk -v block="$block" -v server_name="$SERVER_NAME" '
...
map_config | write_if_changed "$MAP_FILE"
location_config | write_if_changed "$LOCATION_FILE"

if [[ "$MODE" == "existing" ]]; then
  install_include_once "$SERVER_CONF" "$LOCATION_FILE"
else
  validate_new_mode_inputs
  server_config | write_if_changed "$SERVER_FILE"
  if [[ "$DRY_RUN" == "1" ]]; then
...
echo "  MULMOTERMINAL_TAILSCALE_MODE=local MULMOTERMINAL_BASE_PATH=${BASE_PATH} MULMOTERMINAL_ALLOWED_ORIGINS=https://${SERVER_NAME:-<your-existing-nginx-host>} ./scripts/start-tailscale-dev.sh"
